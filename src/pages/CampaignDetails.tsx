...
import { Photo } from "../types"; // ADD THIS IMPORT IF NOT PRESENT
...
const CampaignDetails: React.FC = () => {
  ...
  const [purchasedPhotoIds, setPurchasedPhotoIds] = useState<Set<string>>(
    new Set()
  );
  useEffect(() => {
    if (id) {
  ...
  const handleDownload = async (photo: Photo) => {
    ...
    try {
      // 1. Increment the download count in Supabase by calling the function
      const { error: rpcError } = await supabase.rpc(
        "increment_download_count",
        {
          campaign_id_to_update: id,
        }
      );
      ...
      link.href = photo.url; // Use the full-resolution photo URL
      link.setAttribute(
        "download",
        photo.filename || `goodpix-photo-${photo.id}.jpg`
      );
    ...
    <div className="min-h-screen bg-white">
            ...
            <div className="lg:col-span-2">
              ...
              <div className="flex flex-wrap items-center gap-x-6 gap-y-4 text-gray-700 mb-8">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span>{campaign.download_count || 0} Downloads</span>
                </div>
              </div>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {approvedPhotos.map((photo) => {
                ...
                const isPurchased = purchasedPhotoIds.has(photo.id);
                    ...
                    {isPurchased ? (
                      <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center rounded-lg">
                        <button
                          ...
                          className="bg-orange-500 text-white font-semibold py-2 px-4 rounded-lg flex items-center space-x-2 hover:bg-orange-600 transition-colors"
                        >
                      ...
                      <>
                        <div className="absolute top-2.5 right-2.5 w-6 h-6 bg-white/80 backdrop-blur-sm rounded-md border border-gray-300 flex items-center justify-center pointer-events-none">
                          {isSelected && (
            ...
            {/* THIS IS THE CORRECT LOCATION FOR CAMPAIGN COMMENTS */}
            <div className="mt-16">
      ...
      <PaymentModal
        ...
        onSuccess={() => {
          ...
          setPurchasedPhotoIds((prev) => new Set([...prev, ...selectedPhotos]));
          setSelectedPhotos([]);
          setAdditionalDonation("");
          toast.success(
            "Payment successful! You can now download your photos."
          );
        }}
      />
    </div>
